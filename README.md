...
The backquote key (`` ` ``) drops down a Source-style developer console: `help` lists commands, Tab completes, ↑/↓ recall history. Highlights: `noclip`, `timescale 0.25`, `time 19` (or `time +2`, `time 6:30`), `weather storm`, `daylength 10`, `setpos x y z`, `shot 1..9`, `fog`, `wind`, `fov`, `speed`, `freeze`, `dpr`, `stat`. Any system can register commands via `registerCommand`/`registerCvar` in `src/debug/Console.ts`.

The world lives on a clock and under weather. Time of day advances continuously (default: a full day every 30 real minutes — `daylength` sets it, 0 pins the sun) and re-lights everything as it goes: sun, sky, shadows, GI probes, film grade. Weather wanders a seeded state machine — clear, fair, overcast, fog, rain, storm (and a manual-only global snow) — cross-fading cloud coverage and density, valley fog, wind strength, and precipitation: wind-tilted rain streaks that thin out under forest canopy, snowfall in any biome. `weather storm` pins a state, `weather auto` resumes the wander. Both run on world time, so `timescale` slow-motion slows the sky with it and `?freeze=1` keeps screenshots deterministic.

Useful URL parameters: `?seed=N` (world seed), `?T=hours` (time of day, 0–24), `?shot=1..9` (boot into a bookmark), `?cam=x,y,z,yaw,pitch[,fov]` (exact pose), `?preset=low|high|ultra`, `?freeze=1` (freeze world motion), `?hud=1` (HUD open at boot), `?prof=1` (per-pass GPU timestamps — off by default; the profiling itself costs frame time), `?weather=clear|fair|overcast|fog|rain|storm|snow` (pin a state; `off` disables the system), `?daylen=minutes` (real minutes per 24 h day, 0 = fixed time).
